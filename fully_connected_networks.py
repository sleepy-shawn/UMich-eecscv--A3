...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out1, fr1_cache = Linear_ReLU.forward(X, W1, b1)
        scores, f2_cache = Linear.forward(out1, W2, b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dh, grads['W2'], grads['b2'] = Linear.backward(dout, f2_cache)
        grads['W2'] += 2 * W2 * self.reg

        dx, grads['W1'], grads['b1'] = Linear_ReLU.backward(dh, fr1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers):
          self.params[f'W{i}'] = weight_scale * torch.randn(current_dim, hidden_dims[i-1], dtype=dtype, device=device)
          self.params[f'b{i}'] = torch.zeros(hidden_dims[i-1], dtype=dtype, device=device)
          current_dim = hidden_dims[i-1]

        self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(hidden_dims[self.num_layers - 2], num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
          if i == self.num_layers:
            scores, cache[i] = Linear.forward(current_out, self.params[f'W{i}'], self.params[f'b{i}'])
          else:
            out[i], cache[i] = Linear_ReLU.forward(current_out, self.params[f'W{i}'], self.params[f'b{i}'])
            if self.use_dropout:
              out[i], mask_cache[i] = Dropout.forward(out[i], self.dropout_param)
        ...
        for i in range(self.num_layers, 0, -1):
          if i == self.num_layers:
            dh, grads[f'W{i}'], grads[f'b{i}'] = Linear.backward(dh, cache[i])
          else:
            if self.use_dropout:
              dh = Dropout.backward(dh, mask_cache[i])
            dh, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dh, cache[i])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    batch_size = 64
    lr_decay = 0.95
    solver = Solver(model, data_dict, update_rule = update_rule, optim_config={'learning_rate':1e-2}, 
